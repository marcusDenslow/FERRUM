...
int amend_commit(NCursesDiffViewer *viewer) {
    if (!viewer || viewer->commit_count == 0) return 0;
    ...
    if (get_commit_title_input(new_title, MAX_COMMIT_TITLE_LEN, new_message, sizeof(new_message))) {
        ...
        if (strlen(new_message) > 0) {
            snprintf(amend_cmd, sizeof(amend_cmd), "git commit --amend -m \"%s\" -m \"%s\" 2>/dev/null >/dev/null", new_title, new_message);
        } else {
            snprintf(amend_cmd, sizeof(amend_cmd), "git commit --amend -m \"%s\" 2>/dev/null >/dev/null", new_title);
        }
...
int run_ncurses_diff_viewer(void) {
    ...
    if (viewer.current_mode == NCURSES_MODE_FILE_LIST) {
        ...
        mvprintw(0, 0, "Git Diff Viewer: 1=files 2=view 3=commits | j/k=nav P=push p=pull r/R=reset a=amend | q=quit");
    }
