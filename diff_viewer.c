...
int load_new_file_content(DiffViewer *viewer, const char *filename) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL && viewer->diff_line_count < MAX_DIFF_LINES) {
        // Remove newline
...
void render_diff_viewer(DiffViewer *viewer) {
    ...
    if (viewer->current_mode == MODE_FILE_CONTENT && 
        viewer->file_count > 0 && viewer->selected_file < viewer->file_count) {
        ...
        for (int i = 0; i < available_height && 
             (i + viewer->diff_scroll_offset) < viewer->diff_line_count; i++) {
        ...
        printf(ANSI_COLOR_YELLOW "Select a file and press Enter to view its diff" ANSI_COLOR_RESET);
    }
    ...
    if (viewer->file_count > 0) {
        if (viewer->current_mode == MODE_FILE_LIST) {
            printf("File %d/%d: %s [File List Mode]", 
        ...
        case 66: // Down arrow
            if (viewer->current_mode == MODE_FILE_LIST) {
                ...
                if (viewer->diff_line_count > available_height && 
                    viewer->diff_scroll_offset < viewer->diff_line_count - available_height) {
                    viewer->diff_scroll_offset++;
                }
            }
            break;
