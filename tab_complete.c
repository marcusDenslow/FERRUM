...
/**
 * Get the type of argument expected for a command and if strict matching is required
 */
static ArgumentType get_argument_type(const char *cmd, int *strict_match) {
  if (!cmd || !*cmd) {
    if (strict_match) *strict_match = 0;
    return ARG_TYPE_ANY;
  }
  
  for (int i = 0; command_arg_info[i].command != NULL; i++) {
    if (strcmp(command_arg_info[i].command, cmd) == 0) {
      if (strict_match) *strict_match = command_arg_info[i].strict_match;
 ...
 */
static SuggestionList *get_suggestions_by_type(ArgumentType arg_type, const char *token) {
  ...
  switch (arg_type) {
    ...
    case ARG_TYPE_BOTH: {
      ...
      while ((entry = readdir(dir)) != NULL) {
        ...
        if (entry->d_name[0] == '.' && name_prefix[0] != '.') {
          // Skip hidden files/dirs (except "." and ".." when prefix is empty)
          if (name_prefix_len == 0 && 
              (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0)) {
            // Allow "." and ".." when prefix is empty
          } else {
        ...
        while ((entry = readdir(dir)) != NULL && idx < matched_count) {
          ...
          if (strncasecmp(entry->d_name, name_prefix, name_prefix_len) == 0) {
            ...
            if (last_slash) {
              ...
              // Combine with entry name
              char suggestion_path[PATH_MAX];
              snprintf(suggestion_path, sizeof(suggestion_path), "%s%s%s", 
                      base_path, entry->d_name, is_dir ? "/" : "");
    ...
    case ARG_TYPE_BOOKMARK: {
      ...
      if (bookmarks) {
        ...
        for (int i = 0; i < bookmark_count; i++) {
          if (token[0] == '\0' || strncasecmp(bookmarks[i], token, strlen(token)) == 0) {
            matched_count++;
          }
        }
    ...
    case ARG_TYPE_ALIAS: {
      ...
      if (aliases) {
        ...
        for (int i = 0; i < alias_count; i++) {
          if (token[0] == '\0' || strncasecmp(aliases[i], token, strlen(token)) == 0) {
            matched_count++;
          }
        }
    ...
    case ARG_TYPE_FAVORITE_CITY: {
      ...
      if (cities) {
        ...
        for (int i = 0; i < city_count; i++) {
          if (token[0] == '\0' || strncasecmp(cities[i], token, strlen(token)) == 0) {
            matched_count++;
          }
        }
    ...
    case ARG_TYPE_THEME: {
      ...
      if (themes) {
        ...
        for (int i = 0; i < theme_count; i++) {
          if (token[0] == '\0' || strncasecmp(themes[i], token, strlen(token)) == 0) {
            matched_count++;
          }
        }
 ...
 */
static char *complete_argument_by_type(ArgumentType arg_type, const char *token) {
  if (!token) return NULL;
...
SuggestionList *get_suggestion_list(const char *buffer, const char *prefix) {
  ...
  if (current_context.token_index == 0) {
    ...
    if (matched_count > 0) {
      ...
      SuggestionList *suggestions = (SuggestionList *)malloc(sizeof(SuggestionList));
      if (suggestions) {
        suggestions->items = items;
        suggestions->count = matched_count;
        suggestions->current_index = 0;
  ...
  int strict_match = 0;
  ArgumentType arg_type = get_argument_type(current_context.filter_command, &strict_match);
  
  // Get suggestions for the expected argument type
  SuggestionList *suggestions = get_suggestions_by_type(arg_type, 
      prefix ? prefix : current_context.current_token);
  ...
  // If we're not in strict mode and didn't find suggestions of the expected type,
  // try to fall back to path completions for most types (if they're different)
  if (arg_type != ARG_TYPE_FILE && arg_type != ARG_TYPE_DIRECTORY && arg_type != ARG_TYPE_BOTH) {
    ...
    return get_suggestions_by_type(ARG_TYPE_BOTH, 
        prefix ? prefix : current_context.current_token);
  }
...
char *get_tab_completion(const char *buffer) {
  ...
  char *completion = complete_argument_by_type(arg_type, current_context.current_token);
